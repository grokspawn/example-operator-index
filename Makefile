...
# `render-template composite` has `--validate` option defaulted on, so no subsequent step is required
.PHONY: composite
composite: bin/opm
	bin/opm alpha render-template composite -f catalogs.yaml -c contributions.yaml

.PHONY: tap-fitter
tap-fitter: bin/tap-fitter
	bin/tap-fitter --catalog-path catalogs.yaml --composite-path contributions.yaml --provider ${OPERATOR_NAME}
...
bin/tap-fitter:
	if [ ! -e bin ] ; then mkdir -p bin; fi
	GOBIN=$(CUR_DIR)/bin go install github.com/release-engineering/tap-fitter/cmd/tap-fitter@latest
